    ...
    implements TreeExpansionListener, PropertyChangeListener, RowMapper, TreeSelectionListener, KeyEventDispatcher, KeyEventPostProcessor
{
    ...
    */
	...
	DualTree dTree = new DualTree();
	dTree.getUpperTree().setComponentOrientation(ComponentOrientation.RIGHT_TO_LEFT);
	dTree.setAlignRootVerticalLeg(true);
	
	FlippableTree fTree = new FlippableTree();
	//fTree.setOrientation(Orientation.VERTICAL);
	//fTree.setOrientation(Orientation.NONE);
	fTree.setComponentOrientation(ComponentOrientation.RIGHT_TO_LEFT);
	fTree.setShowsRootHandles(true);	
	fTree.setShowsRootVerticalLineLeg(true);
	//fTree.setRootVisible(false);	
	
	
	//JTree dTree = new JTree();
	//dTree.setComponentOrientation(ComponentOrientation.RIGHT_TO_LEFT);
	//dTree.setRootVisible(false);	
	dTree.setShowsRootHandles(true);
	//dTree.setShowsRootHandles(false);
	...
	frame.getContentPane().add(dTree);
	//frame.getContentPane().add(fTree);
	//frame.getContentPane().add(dTree.getLowerTree());
	//dTree.getLowerTree().setOffsetX(100);	
	...
	*/
	...
	//	SwingUtilities.updateComponentTreeUI(dTree);	
    }
