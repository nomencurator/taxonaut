/*
 * MotifAlignableTreeUI.java:  a TreeUI with node alignment between trees
 *
...
/**
 * <CODE>MotifAlignableTreeUI</CODE> provides a <code>TreeUI</code> capable node
 * alignemt between trees using <CODE>Alinger</CODE>
 *
 * @version 	10 July 2015
 * @author 	Nozomi `James' Ytow
 */
public class MotifAlignableTreeUI
    extends MotifTreeUI
    implements Alignable, AlignableTreeUI
{
    //    Aligner aligner;

    public MotifAlignableTreeUI() {
	super();
    }

    /*
    public static ComponentUI createUI(JComponent x) {
	if(x != null && x instanceof Alignable)
	    return new MotifAlignableTreeUI(((Alignable)x).getAligner());
        return new MotifAlignableTreeUI();
    }
    */
