/*
 * ArticlePanel.java:  a java based GUI for Nomencurator
 *
...
/**
 * {@code ArticlePanel} provides {@code CitationPanel}
...
{
    private static final long serialVersionUID = 8039212491115037049L;

    public ArticlePanel(CitationModel model,
			    Locale locale)
    {
	super(model, locale);
    }
	...
	if(layoutComponent(contentsLabel, contentsTitle, layout, constraints))
	    constraints.gridy++;
	...
	if(layoutComponent(receivedLabel, received, layout, constraints))
	    constraints.gridy++;

	if(layoutComponent(revisedLabel, revised, layout, constraints))
	    constraints.gridy++;

	if(layoutComponent(acceptedLabel, accepted, layout, constraints))
	    constraints.gridy++;

	if(layoutComponent(publishedLabel, published, layout, constraints))
	    constraints.gridy++;

	if(layoutComponent(doiLabel, doi, layout, constraints))
	    constraints.gridy++;
	...
	String citationText =
	    ResourceKey.JOURNAL_CITATION_LABEL;
	String isxnText =
	    ResourceKey.ISSN;
